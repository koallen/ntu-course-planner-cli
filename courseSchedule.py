        ...
        for i in range(len(schedule) // 7):
            for j in range(7):
                ...
                if j == 0 and string != "": # get the index of the course
                    ...
                    indexOfDay = CourseSchedule.indexOfDay[string]
                elif j == 4: # get the time of that course in a day
                    numOfTimeSlots = self.__getTime(string)
                    indexOfTime    = (int(string[:4]) - 830) // 50
                # if j reaches 6, prasing is done
                # the following code is to modify the binary string according to the parsing result
                elif j == 6:
                    startingIndex = indexOfDay + indexOfTime
                    endingIndex   = startingIndex + numOfTimeSlots
                    self.__schedule[currentIndex] = self.__schedule[currentIndex][:startingIndex] + \
                                                    "1" * numOfTimeSlots + \
                                                    self.__schedule[currentIndex][endingIndex:]
                else:
                    pass

    def __fetchSchedule(self, courseCode):
